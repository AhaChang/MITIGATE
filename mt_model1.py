...
def train_nc_model(args, model_nc, opt_nc, features, adj, labels, ano_labels, idx_train_nc, idx_train_ad, idx_val, filename):
    ...
    for epoch in range(args.max_epoch):
        ...
        with torch.no_grad():
            ...
            nc_idacc_val = accuracy(prob_nc[idx_val], labels[idx_val])

            print('Train Loss', "{:.5f}".format(loss.item()),
                    'NC-ACC:', "{:.5f}".format(nc_idacc_val))
            ...
            if cur_p > patience or epoch+1 >= args.max_epoch:
                print('epoch: {}, acc_val: {}, best_acc_val: {}'.format(epoch, nc_idacc_val, best_val))
                # load best model
...
def train_ad_model(args, model_ad, opt_ad, prob_nc, features, adj, ano_labels, idx_train_ad, idx_val, filename):
    ...
    for epoch in range(args.max_epoch):
        ...
        embed, prob_ad = model_ad(features, adj, prob_nc)
        ...
        with torch.no_grad():
            ...
            if cur_p > patience or epoch+1 >= args.max_epoch:
                ...
                # load best model
                checkpoint = torch.load(filename+'_checkpoint_m1.pt')
...
def test_ad_model(model_ad, features, adj, ano_labels, prob_nc, idx_train_ad, idx_test):
    with torch.no_grad():
        model_ad.eval()
        embed_ad, prob_ad = model_ad(features, adj, prob_nc)
        ...
        print('ACC', "{:.5f}".format(test_acc_ad), 
            ...
            'Pre', "{:.5f}".format(test_pre_ad),
            'Rec', "{:.5f}".format(test_rec_ad),
            'AUC', "{:.5f}".format(test_auc_ad),
            'N_num', normal_num,
            'A_num', abnormal_num)
        
    return embed_ad, prob_ad, test_auc_ad, test_f1macro_ad, test_f1micro_ad, test_pre_ad, test_rec_ad, abnormal_num, normal_num
...
def main(args):
    ...
    model_ad = ADModel_1(ft_size, args.embedding_dim, 2, nb_classes, dropout=0.6)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--strategy_ad', type=str, default='topk_medoids_3') # random uncertainty topk_anomaly
    ...
    parser.add_argument('--loss', type=str, default='sum_div')
