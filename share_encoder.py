...
def main(args):
    ...
    timestamp = time.time()
    prefix = 'saved_models/MultiTask/' + args.dataset + str(args.device) + '/'
    ...
    for iter in range(args.iter_num + 1):
        ...
        if args.beta > 1:
            budget_nc = int(nb_classes * args.nc_num)
        else:
            budget_nc = int(nb_classes * args.nc_num * (args.beta ** iter))
        ...
        if len(idx_train_ad) < args.max_budget * 2:
            ...
            elif args.strategy_nc == 't7':
                idx_selected_nc = query_t7(adj, prob_nc, prob_ad, budget_nc, idx_cand_nc.tolist())
            elif args.strategy_nc == 'ano_entropy':
                idx_selected_nc = query_entropy(prob_ad, budget_nc, idx_cand_nc.tolist())
    ...
    if not os.path.exists(des_path):
        with open(des_path,'w+') as f:
            ...
            csv_head = ["model", "seed", "dataset", "lr","embedding_dim", "init_num", "num_epochs", "nc_num", "beta", "strategy_ad", "strategy_nc", "alpha", "nc-acc", "nc-idacc", "nc-f1-micro", "nc-f1-macro", "ad-auc", "ad-f1-macro", "A-num", "N-num"]
            csv_write.writerow(csv_head)

    with open(des_path, 'a+') as f:
        csv_write = csv.writer(f)
        data_row = ['m1', args.seed, args.dataset, args.lr, args.embedding_dim, args.init_num, args.max_epoch, args.nc_num, args.beta, args.strategy_ad, args.strategy_nc, args.alpha, test_acc_comm, test_acc_nc_id, test_f1micro_comm, test_f1macro_comm, test_auc_ano, test_f1macro_ano, abnormal_num, normal_num]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--beta', type=float, default=0.8, help='time sensitive parameter')
    parser.add_argument('--strategy_nc', type=str, default='ano_entropy') # random uncertainty largest_degree featprop
    ...
    parser.add_argument('--nc_num', type=int, default=200)
