...
def train_nc_model(args, model_nc, opt_nc, features, adj, labels, ano_labels, idx_train_nc, idx_train_ad, idx_val, filename):
    ...
    for epoch in range(args.max_epoch):
        ...
        with torch.no_grad():
            ...
            val_loss_sup = xent(prob_nc[idx_val_id], labels[idx_val_id])
            ...
            if cur_p > patience or epoch+1 >= args.max_epoch:
                print('epoch: {}, acc_val: {}, best_idacc_val: {}'.format(epoch, nc_idacc_val, best_val))
                # load best model
...
def train_ad_model(args, model_ad, opt_ad, prob_nc, features, adj, ano_labels, idx_train_ad, idx_val, filename):
    ...
    for epoch in range(args.max_epoch):
        ...
        prob_ad = model_ad(features, adj, prob_nc)
        ...
        with torch.no_grad():
            ...
            if cur_p > patience or epoch+1 >= args.max_epoch:
                print('epoch: {}, acc_val: {}, best_epoch: {}, best_acc_val: {}'.format(epoch, ad_auc_val, best_iter, best_val))
                # load best model
...
def test_ad_model(model_ad, features, adj, ano_labels, prob_nc, idx_train_ad, idx_test):
    ...
    return prob_ad, test_auc_ano, test_f1macro_ano, test_f1micro_ano, test_pre, test_rec, abnormal_num, normal_num
...
def main(args):
    ...
    model_ad = ADModel_2(ft_size, args.embedding_dim, 2, nb_classes, dropout=0.6)
    ...
    if not os.path.exists(prefix):
        os.makedirs(prefix)
    filename = prefix + str(args.seed)+ '_ad1_'+ str(timestamp)
    ...
    for iter in range(args.iter_num + 1):
        ...
        # Train node classification model
        model_nc, opt_nc = train_nc_model(args, model_nc, opt_nc, features, adj, labels, ano_labels, idx_train_nc, idx_train_ad, idx_val, filename)
        ...
        # Train anomaly detection model
        model_ad, opt_ad = train_ad_model(args, model_ad, opt_ad, prob_nc, embed_nc, adj, ano_labels, idx_train_ad, idx_val, filename)
        prob_ad, test_auc_ano, test_f1macro_ano, test_f1micro_ano, test_pre, test_rec, abnormal_num, normal_num = test_ad_model(model_ad, embed_nc, adj, ano_labels, prob_nc, idx_train_ad, idx_test)
        ...
        if len(idx_train_ad) < args.max_budget * 2:
            ...
            if budget_ad != 0:
                ...
                elif args.strategy_ad == 'mix_entropy':
                    idx_selected_ad = query_entropy(torch.cat((prob_nc,prob_ad),dim=1), budget_ad, idx_cand_an.tolist())
                ...
                #     idx_selected_ad = query_topk_medoids(embed_nc+embed_ad, prob_ad, budget_ad, idx_cand_an.tolist(), nb_classes)
                    ...
                    idx_selected_ad = query_topk_medoids(torch.cat((embed_nc,prob_nc),dim=1), prob_ad, budget_ad, idx_cand_an.tolist(), nb_classes)
                ...
                #     idx_selected_ad = query_topk_medoids_m(embed_nc+embed_ad, prob_ad, budget_ad, idx_cand_an.tolist(), nb_classes)
                    ...
                    idx_selected_ad = query_topk_medoids_m(torch.cat((embed_nc,prob_nc),dim=1), prob_ad, budget_ad, idx_cand_an.tolist(), nb_classes)    
...
if __name__ == '__main__':
    ...
    parser.add_argument('--strategy_ad', type=str, default='mix_entropy') # random uncertainty topk_anomaly
