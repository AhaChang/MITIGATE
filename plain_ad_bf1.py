...
def train_ad_model(args, model_ad, opt_ad, features, adj, ano_labels, idx_train_ad, idx_val, filename):
    ...
    for epoch in range(args.max_epoch):
        ...
        with torch.no_grad():
            ...
            val_loss = F.cross_entropy(prob_ad[idx_val], ano_labels[idx_val], weight=torch.tensor([1., weight]).cuda())
            ...
            # Save model untill metrics does not increase
            if epoch > early_stopping:
                if ad_f1macro_val > best_val:
                    ...
                    }, filename+'_checkpoint_plain_ad_bf1.pt')
                else:
                    cur_p += 1
            ...
            if cur_p > patience or epoch+1 >= args.max_epoch:
                ...
                # load best model
                checkpoint = torch.load(filename+'_checkpoint_plain_ad_bf1.pt')
...
def main(args):
    ...
    timestamp = time.time()
    prefix = 'saved_models/plain/' + args.dataset + str(args.device) + '/'
    if not os.path.exists(prefix):
        os.makedirs(prefix)
    filename = prefix + str(args.seed)+ '_plain_ad_bf1_'+ str(timestamp)
        ...
        if len(idx_train_ad) < args.max_budget * 2:
            ...
            if budget_ad != 0:
                if args.strategy_ad == 'random':
                    ...
                    idx_selected_ad = query_topk_medoids_m(embed_ad, prob_ad, budget_ad, idx_cand_an.tolist(), nb_classes)
    ...
    if not os.path.exists(des_path):
        with open(des_path,'w+') as f:
            ...
            csv_head = ["model", "seed", "dataset", "init_num", "num_epochs", "strategy_ad", "ad-auc", "ad-f1-macro","ad-pre-macro","ad-rec-macro", "A-num", "N-num"]
            csv_write.writerow(csv_head)

    with open(des_path, 'a+') as f:
        csv_write = csv.writer(f)
        data_row = ['plain_ad_bf1', args.seed, args.dataset, args.init_num, args.max_epoch, args.strategy_ad, test_auc_ano, test_f1macro_ano, test_pre, test_rec, abnormal_num, normal_num]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', type=str, default='citeseer')  # 'BlogCatalog'  'Flickr'  'ACM'  'cora'  'citeseer'  'pubmed'
    ...
    parser.add_argument('--result_path', type=str, default='results/plain_ad_bf1')
